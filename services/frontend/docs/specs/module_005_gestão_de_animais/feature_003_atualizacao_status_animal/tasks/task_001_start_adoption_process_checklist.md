# Makuco Codegen Checklist: TASK-BACKEND-001

**Purpose**: Validate the quality of the state machine infrastructure and start-adoption-process endpoint implementation.
**Created**: 2026-06-02
**Feature**: [spec_context.md](../spec_context.md)
**Prompt Plan**: task_001_start_adoption_process.md
...
- MCP tools (sonar-run, complexity-check, quality-check) were unavailable due to server startup failure. Validation was done manually via CLI tools (`npx tsc --noEmit`, `npx eslint`, `npx jest`).
- The `revertToAvailable` method uses an explicit status guard (`!== 'in_adoption_process'`) instead of generic `validateTransition` to comply with acceptance criteria that `adopted → available` should be rejected for this specific endpoint, despite `VALID_TRANSITIONS` allowing it generically.
